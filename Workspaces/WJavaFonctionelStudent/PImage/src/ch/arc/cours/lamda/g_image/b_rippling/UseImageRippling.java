...
//		int[] tabARGB = createArrayPixel_v2(w, h);
		...
		chrono.stop();
		System.out.println("[Rippling] : "+chrono);
	...
	private static int[] createArrayPixel_v1(int w, int h)
		{
		ColorieurRippling colorieur = new ColorieurRippling(w, h);
	...
	private static int[] createArrayPixel_v2(int w, int h)
		...
		IntStream.range(0, w*h).parallel().forEach(i -> tab[i] = colorieur.color(i));
	...
	private static int[] createArrayPixel_v3(int w, int h)
		...
		return IntStream.range(0, w*h).parallel().map(colorieur::color).toArray();
		}
